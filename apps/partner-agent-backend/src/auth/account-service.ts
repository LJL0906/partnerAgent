...
import { AccountStore, type Account, type LoginSession } from './account-store.js';
import {
  AccountRateLimit,
  credentials,
  hashPassword,
...
export class AccountService {
  ...
  async register(body: unknown, ip: string): Promise<AccountTokens> {
    ...
    if (!(await this.store.create(account)))
      throw new ConflictException('用户名已被占用，请更换一个用户名。');
  ...
  async me(id: string): Promise<{ id: string; username: string }> {
    ...
    if (!account) throw new UnauthorizedException('该身份尚未绑定登录account。');
