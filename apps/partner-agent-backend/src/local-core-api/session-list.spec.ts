...
describe('session-list与恢复', () => {
  ...
  it('archives a session and excludes it from the active list', async () => {
    const sessions = new MemorySessionStore();
    const app = new LocalCoreApplicationService(
      sessions,
      {} as ConfirmationTransactionService,
      new MemoryChatTaskStore(sessions),
      {} as ChatTaskScheduler,
      {} as PrivacyDecisionService,
    );
    await sessions.createIfAllowed('keep', 'a', 10);
    await sessions.createIfAllowed('archive', 'a', 10);
    ...
    expect(archived).toMatchObject({ id: 'archive' });
    expect((await sessions.find('archive', 'a'))?.archivedAt).toBeInstanceOf(Date);
    expect((await sessions.list('a')).map((session) => session.id)).toEqual(['keep']);
  });
