...
当前仓库同时保存产品需求、架构设计和产品自有代码。Monorepo、共享契约、持久会话、隐私外发决定闭环、外发审计、确认事务、正式 WS v1、Expo 聊天主链路和 Action 纵切已经落地；清晰单一的行动意图会直接经审计事务写入，不确定或多候选意图才保留聊天内预览供用户选择。所有消息型卡片按聊天时间线展示；复杂任务提供随任务自动清空的临时待办浮层，简单问答不创建待办。
...
- 聊天 Action 已按“单候选、置信度不低于 0.85、无不确定性说明”自动复用该原子事务写入；不确定或多个互斥候选通过聊天中的单一分步选择浮层继续补充或选择，批次内候选以单选项展示，多批次支持前后翻页，最后一项选择即作为最终授权自动汇总给 Agent 执行，不再二次确认。同一任务的正式候选与结构化预览不重复投影；模型思考摘要随助手消息持久化并可在恢复后追溯；
...
3. 继续验证聊天行动自动写入与不确定/多候选澄清两条路径；正式对象仍只能通过统一审计事务生效；
